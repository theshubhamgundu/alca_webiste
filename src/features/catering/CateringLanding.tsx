...
const logoImg = "/images/alca-logo-1.webp";
...
/** Floating pill navbar used on both the main page and the menu page */
function PillNavbar({
  mobileOpen,
  setMobileOpen,
}: {
  ...
  return (
    <div className="fixed top-3 md:top-4 left-4 right-4 md:left-8 md:right-8 z-50 flex justify-center">
      <nav className="w-full max-w-5xl bg-[#FAF7F2] rounded-full shadow-[0_15px_50px_-10px_rgba(0,0,0,0.35)] px-4 md:px-6 py-2 flex items-center justify-between">
        {/* Logo + wordmark */}
        <a href="#home" aria-label="Return to homepage" className="flex items-center gap-2 shrink-0">
          <img src={logoImg} alt="ALCA logo" className="h-9 w-auto" />
          <span className="font-display text-lg font-bold text-[#2C1A0E] tracking-wide">
            ALCA Caterers
          </span>
        </a>
        ...
        <div className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((l) => (
            <a
              key={l.label}
              href={l.href}
              className="text-sm font-semibold tracking-wide text-[#2C1A0E] uppercase hover:text-[#7B1E1E] transition-colors"
            >
              {l.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-3">
          <a
            href={createWhatsAppUrl("Hello ALCA Caterers, I would like to discuss catering options for my event.")}
            target="_blank"
            rel="noopener"
            className="hidden sm:inline-flex bg-[#7B1E1E] text-[#FAF7F2] rounded-full px-5 py-2 text-sm font-semibold hover:bg-[#9B4A1E] transition-colors"
          >
            WhatsApp us
          </a>
          <button
            className="md:hidden text-[#2C1A0E] text-xl px-2"
...
export default function CateringLanding() {
  ...
  if (isMenuPage) {
    return (
      <div className="catering-page min-h-full bg-[#FAF7F2] text-[#2C1A0E]">
        <PillNavbar mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} />
  ...
  return (
    <div className="catering-page min-h-full bg-[#FAF7F2] text-[#2C1A0E] overflow-clip">

      {/* PILL NAVBAR */}
      <PillNavbar mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} />
      {/* Spacer so page content starts below the fixed pill nav */}
      <div className="h-16 md:h-20" />
      ...
      {/* HERO */}
      <section id="home" className="relative min-h-[calc(100svh-8rem)] md:min-h-screen flex items-center overflow-hidden mandala-tile">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-10 grid md:grid-cols-2 gap-10 md:gap-12 items-center py-10 md:py-20">
          <div className="text-center md:text-left">
            ...
            <h1 className="font-display text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-tight text-[#2C1A0E] mb-4">
              Welcome to<br />
              <span className="text-[#7B1E1E] italic">ALCA Caterers</span>
            </h1>

            <p className="text-[#5C3D2E] text-sm sm:text-base leading-relaxed mb-6 md:mb-8 max-w-lg mx-auto md:mx-0">
              ALCA Caterers delivers reliable event catering for weddings, corporate functions, family gatherings, and special occasions.
            </p>
      ...
      <footer id="contact" className="py-14 px-6 md:px-10 bg-[#2C1A0E] border-t border-[#C9A96E20]">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-12">
          ...
          <div className="flex flex-col items-center md:items-start gap-4">
            <div className="flex items-center gap-2">
              <img src={logoImg} alt="ALCA Caterers" className="h-14 object-contain" />
              <span className="text-xs font-semibold tracking-[0.3em] uppercase text-[#C9A96E]">ALCA CATERERS</span>
            </div>
          ...
          <div className="flex flex-col items-center md:items-start gap-4">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                ...
                <p className="text-xs text-[#FAF7F2CC] leading-relaxed">Hyderabad, Telangana — 500041</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-sm mt-0.5">📞</span>
                <a href={createWhatsAppUrl("Hello ALCA Caterers, I would like to discuss catering options for my event.")} target="_blank" rel="noreferrer" className="text-xs text-[#FAF7F2CC] leading-relaxed hover:text-[#25D366] transition-colors">
                  WhatsApp: +91 90109 95180
                </a>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-sm mt-0.5">✉️</span>
                <p className="text-xs text-[#FAF7F2CC] leading-relaxed">info@ALCAcaterers.in</p>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto mt-10 pt-6 border-t border-[#C9A96E15] text-center">
          <p className="text-[10px] text-[#FAF7F260] tracking-wider">
            © 2026 ALCA Caterers, Hyderabad. All rights reserved.
          </p>
